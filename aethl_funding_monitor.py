...
try:
    import pypdf            # optional: read Grants.gov announcement PDFs
except Exception:
    pypdf = None
import io
...
STRONG_TERMS = [
    "wound", "wound care", "wound healing", "wound dressing", "chronic wound",
    "diabetic foot", "foot ulcer", "leg ulcer", "pressure ulcer", "pressure injury",
    "burn", "burn injury", "hydrogel", "tissue oxygen", "tissue oxygenation",
    "dissolved oxygen", "oxygenation", "hypoxia", "hypoxic", "hyperbaric",
    "oxygen therapy", "limb salvage", "regenerative", "tissue regeneration",
    "skin substitute", "wound matrix", "negative pressure wound", "debridement",
    "biofilm", "combat casualty", "trauma wound", "dfu",
]
MEDIUM_TERMS = [
    "biomaterial", "drug delivery", "nitric oxide", "dermatology", "skin repair",
    "antimicrobial", "medical device", "tissue repair", "epithelial",
    "epithelialization", "granulation", "angiogenesis", "ischemia", "reperfusion",
    "perfusion", "prolonged field care", "field care", "casualty care",
    "point of injury", "far forward", "austere", "soft tissue injury",
    "extremity injury", "blast injury", "hemorrhage", "scar", "dermal",
    "infection control", "veterinary wound",
]

# Search keywords sent to keyword-based APIs (Grants.gov, SEDIA). SAM.gov keeps
# its own shorter list (it is rate-limited). Broadened to Aethl's full vocabulary.
SEARCH_KEYWORDS = [
    "wound care", "wound healing", "chronic wound", "wound dressing", "wound infection",
    "diabetic foot ulcer", "pressure ulcer", "pressure injury", "leg ulcer",
    "burn injury", "burn wound", "hydrogel", "tissue oxygenation", "hypoxic wound",
    "hyperbaric oxygen", "oxygen therapy", "limb salvage", "regenerative medicine",
    "tissue regeneration", "skin substitute", "skin graft", "wound matrix",
    "negative pressure wound", "debridement", "biofilm", "advanced wound care",
    "soft tissue injury", "extremity trauma", "combat casualty care",
    "prolonged field care", "trauma care", "surgical site infection", "amputation",
    "antimicrobial wound", "nitric oxide", "drug delivery", "biomaterial",
    "dermatology", "scar", "veterinary wound",
    "oral health", "periodontal", "gum disease", "skincare", "anti-aging",
]
...
ENRICH = True
ENRICH_CAP = 60
PDF_CAP = int(os.environ.get("PDF_CAP", "40"))   # max Grants.gov PDFs read per run
PDF_READS = {"n": 0}

# LLM extraction layer (fills funding/#awards/eligibility/soft-scores from
# announcement text, and judges true relevance).
# Provider priority: Groq (free, no billing) -> Gemini -> Anthropic.
GROQ_API_KEY = os.environ.get("GROQ_API_KEY")
GROQ_MODELS = ([os.environ["GROQ_MODEL"]] if os.environ.get("GROQ_MODEL") else []) + [
...
# Candidate Gemini models tried in order until one works on the user's free
# account (auto-fallback; user never has to pick). GEMINI_MODEL env can override.
GEMINI_MODELS = ([os.environ["GEMINI_MODEL"]] if os.environ.get("GEMINI_MODEL") else []) + [
    "gemini-2.0-flash-lite", "gemini-2.0-flash", "gemini-1.5-flash", "gemini-flash-latest",
]
_GEMINI_OK = {"model": None}  # cached working model for this run
USE_LLM = bool(GROQ_API_KEY or GEMINI_API_KEY or os.environ.get("ANTHROPIC_API_KEY"))
...
def to_int(v):
    """Best-effort int; returns None for None/'', 'none', non-numeric."""
    if v is None:
        return None
    s = str(v).strip().replace(",", "").replace("$", "")
    if s == "" or s.lower() in ("none", "null", "n/a", "tbd"):
        return None
    try:
        return int(float(s))
...
def _open(req, timeout=45, tries=2):
    """urlopen with brief backoff on HTTP 429; fails fast so a hard cap
...
def fetch_grants_gov(rows=50):
    ...
    for kw in SEARCH_KEYWORDS:
        try:
            ...
            for h in hits:
                if not isinstance(h, dict) or h.get("id") in seen:
                    continue
                seen.add(h["id"])
                op = make_opp(
            ...
            pass  # individual keyword failure is non-fatal
    return out
        ...
        try:
            ...
            for r in d.get("results", []):
                ...
                if status and status not in ("open", "forthcoming"):
                    continue
                # also drop stale/closed calls by date (some have no status)
                dl_raw = (meta.get("deadlineDate") or meta.get("callDeadlineDate")
                          or meta.get("es_SortDate") or meta.get("startDate"))
                dl_date = None
                if dl_raw:
                    s = str(dl_raw)[:10]
                    for fmt in ("%Y-%m-%d", "%d-%m-%Y", "%m/%d/%Y"):
                        try:
                            dl_date = dt.datetime.strptime(s, fmt).date()
                            break
                        except Exception:
                            pass
                if dl_date and dl_date < dt.date.today():
                    continue  # call already closed
                out.append(make_opp(
                    "EU Funding & Tenders", ref,
                    meta.get("title") or r.get("title") or r.get("summary"),
                    "European Commission (Horizon/EIC)", "EU", "grant/call",
                    "EU-consortium",
                    meta.get("deadlineDate") or meta.get("callDeadlineDate")
                    or meta.get("es_SortDate") or meta.get("startDate"),
...
def fetch_ted(limit=25):
    ...
    try:
        ...
        pass  # TED is best-effort/optional; ignore quietly
    return out
    ...
    for o in opps:
        ...
        try:
            ...
            d = _post_json("https://api.grants.gov/v1/api/fetchOpportunity",
                           {"opportunityId": oid}).get("data", {})
            syn = d.get("synopsis", {}) or {}
            funding = to_int(syn.get("estimatedFunding"))
            naward = to_int(syn.get("numberOfAwards"))
            ceil = to_int(syn.get("awardCeiling"))
            o["total_funding"] = funding
            o["num_awards"] = naward
            if ceil:
                o["per_award"] = ceil
            elif funding and naward:
                o["per_award"] = int(funding / naward)
            elif funding:
                o["per_award"] = funding
            ...
            # If funding is still unknown, read the attached announcement PDF
            # (the numbers are often there even when the synopsis omits them).
            if pypdf and not o.get("total_funding") and PDF_READS["n"] < PDF_CAP:
                pdf_id = _pick_announcement_pdf(d.get("synopsisAttachmentFolders"))
                if pdf_id:
                    try:
                        excerpt = _grants_pdf_excerpt(pdf_id)
                        if excerpt:
                            o["summary"] = (excerpt + " " + (o.get("summary") or ""))[:5000]
                            PDF_READS["n"] += 1
                    except Exception:
                        pass
            n += 1
            ...
            pass  # detail missing/unavailable for this opp -> leave fields blank


def enrich_nih_pages(opps):
    """For opps linking to an NIH Guide page, pull the Funds Available / Award
    Budget / Key Dates / Eligibility sections so the LLM can extract the numbers.
    NIH pages are open (no key); best-effort and capped."""
    if not ENRICH:
        return
    n = 0
    for o in opps:
        link = o.get("link") or ""
        if "grants.nih.gov/grants/guide" not in link or n >= 15:
            continue
        try:
            req = request.Request(link, headers={"User-Agent": USER_AGENT})
            html = _open(req, timeout=30).read().decode("utf-8", "ignore")
            txt = re.sub(r"\s+", " ", re.sub(r"<[^>]+>", " ", html))
            chunks = []
            for kw in ("Funds Available", "Award Budget", "Number of Awards",
                       "Application Due", "Key Dates", "Eligible"):
                i = txt.find(kw)
                if i >= 0:
                    chunks.append(txt[i:i + 700])
            if chunks:
                o["summary"] = (" ".join(chunks))[:5000]
                n += 1
            time.sleep(0.2)
        except Exception:
            pass  # page unavailable -> leave as-is


def _pick_announcement_pdf(folders):
    """Choose the main announcement PDF from a Grants.gov opportunity's
    attachments, preferring the full announcement and skipping forms/FAQs."""
    cands = []
    for f in (folders or []):
        for a in (f.get("synopsisAttachments") or []):
            if a.get("mimeType") != "application/pdf" or not a.get("id"):
                continue
            name = f"{a.get('fileName','')} {a.get('fileDescription','')}".lower()
            score = 0.0
            for good in ("baa", "announcement", "nofo", "foa", "solicitation",
                         "full", "opportunity", "funding", "program"):
                if good in name:
                    score += 3
            for bad in ("form", "template", "instruction", "faq", "q&a", "q and a",
                        "cover", "sf424", "sf-424", "sf 424", "appendix", "checklist",
                        "errata", "amend", "modification", "certification", "budget form"):
                if bad in name:
                    score -= 4
            score += min((a.get("fileLobSize") or 0) / 500000.0, 6)  # bigger = likelier main doc
...
def _grants_pdf_excerpt(attachment_id, page_cap=60):
    """Download a Grants.gov attachment PDF and return an excerpt focused on the
    funding/award sections, so the LLM can extract numbers the synopsis omits."""
    if not pypdf:
        return ""
    url = (f"https://apply07.grants.gov/grantsws/rest/opportunity/att/download/"
           f"{attachment_id}")
    req = request.Request(url, headers={"User-Agent": USER_AGENT})
    raw = _open(req, timeout=60).read()
    reader = pypdf.PdfReader(io.BytesIO(raw))
    txt = ""
    for p in reader.pages[:page_cap]:
        try:
    ...
    txt = re.sub(r"\s+", " ", txt)
    chunks = []
    for kw in ("Award Amount", "Number of Awards", "Funds Available",
               "Total Funding", "Award Ceiling", "Estimated", "Period of Performance",
               "Phase I", "anticipated"):
        i = txt.find(kw)
        if i >= 0:
...
def _groq_call(prompt, key):
    """Call Groq (OpenAI-compatible, free tier), auto-selecting a working model."""
    models = [_GROQ_OK["model"]] if _GROQ_OK["model"] else GROQ_MODELS
    last_err = None
    for m in models:
        ...
        req = request.Request("https://api.groq.com/openai/v1/chat/completions",
                              ...
                              headers={"Authorization": f"Bearer {key}",
                                       ...
                                       "User-Agent": USER_AGENT})
        try:
            resp = json.loads(_open(req, timeout=45).read().decode())
            _GROQ_OK["model"] = m
...
def _gemini_call(prompt, gkey):
    """Call Gemini, auto-selecting the first model that works on this account."""
    body = {"contents": [{"parts": [{"text": prompt}]}],
            "generationConfig": {"maxOutputTokens": 500, "temperature": 0,
                                 "responseMimeType": "application/json"}}
    data = json.dumps(body).encode()
    # if we already found a working model this run, use it
    models = [_GEMINI_OK["model"]] if _GEMINI_OK["model"] else GEMINI_MODELS
    last_err = None
    for m in models:
        url = (f"https://generativelanguage.googleapis.com/v1beta/models/"
               f"{m}:generateContent?key={gkey}")
        req = request.Request(url, data=data,
                              headers={"Content-Type": "application/json",
                                       "User-Agent": USER_AGENT})
        try:
            resp = json.loads(_open(req, timeout=45).read().decode())
            _GEMINI_OK["model"] = m  # remember the one that worked
            return "".join(p.get("text", "")
                           for p in resp["candidates"][0]["content"]["parts"])
        except error.HTTPError as e:
            last_err = e
            if e.code in (404, 429, 400):   # model unavailable/capped -> try next
                continue
            raise
    if last_err:
        raise last_err
...
def _llm_json(prompt):
    """Send a prompt to Groq (free) -> Gemini -> Anthropic; return raw text."""
    if os.environ.get("GROQ_API_KEY"):
        return _groq_call(prompt, os.environ["GROQ_API_KEY"])
    gkey = os.environ.get("GEMINI_API_KEY")
    if gkey:
        return _gemini_call(prompt, gkey)
...
def llm_enrich(opps):
    ...
    if not key:
        print("  [LLM] no GEMINI_API_KEY/ANTHROPIC_API_KEY -> skipping LLM layer",
              file=sys.stderr)
        return
    provider = ("Groq" if os.environ.get("GROQ_API_KEY")
                else "Gemini" if os.environ.get("GEMINI_API_KEY") else "Anthropic")
    n = 0
    consecutive_fail = 0
    for o in opps:
        if n >= LLM_CAP:
        ...
        try:
            ...
            if consecutive_fail >= 3:
                # account-wide cap or persistent error: stop quietly, keep rule-based scores
                break
            continue
        consecutive_fail = 0
        ...
        n += 1
        time.sleep(2.0)
    model_used = _GROQ_OK["model"] or _GEMINI_OK["model"] or LLM_MODEL
    if n:
        print(f"  [LLM] enriched {n} opportunities via {provider} ({model_used})",
              file=sys.stderr)
    else:
        print(f"  [LLM] {provider} unavailable (rate-limited) -> kept rule-based "
              f"scores; tracker still produced", file=sys.stderr)


def fetch_nih_rss():
    """NIH Guide weekly funding opportunities (RFAs/PAs/Notices), filtered to
    Aethl-relevant items so it doesn't flood the sheet."""
    import xml.etree.ElementTree as ET
    out = []
    terms = [t.lower() for t in (STRONG_TERMS + MEDIUM_TERMS)]
    try:
        req = request.Request("https://grants.nih.gov/grants/guide/newsfeed/fundingopps.xml",
                              headers={"User-Agent": USER_AGENT})
        raw = _open(req, timeout=40).read().decode("latin-1", "ignore")
        ...
        for it in root.iter("item"):
            ...
            if not any(t in blob for t in terms):
                continue
            m = re.search(r"\b(?:RFA|PAR|PAS|PA|NOT|NOSI|NOFO)-[A-Z]{2}-\d{2}-\d+\b",
                          title + " " + link)
            num = m.group(0) if m else link
            op = make_opp("NIH Guide", num, title, "HHS - NIH", "United States",
                          "grant", "US-direct", "", link, desc)
            op["number"] = num
            out.append(op)
    except Exception:
        pass  # best-effort source
    return out


def fetch_sbir():
    """SBIR.gov cross-agency SBIR/STTR solicitations. Best-effort: the public
    API is often rate-limited; returns [] quietly if unavailable."""
    out = []
    for kw in ["wound", "burn", "diabetic", "tissue"]:
        try:
            d = _get_json(f"https://api.www.sbir.gov/public/api/solicitations"
                          f"?keyword={parse.quote(kw)}&rows=25")
            if isinstance(d, dict) and d.get("Code"):  # error/throttle payload
                break
            items = d if isinstance(d, list) else d.get("solicitations", [])
            for s in items:
                title = s.get("solicitation_title") or s.get("title") or ""
                num = s.get("solicitation_number") or title
                link = (s.get("sbir_solicitation_link") or s.get("solicitation_link")
                        or "https://www.sbir.gov/solicitations")
                op = make_opp("SBIR.gov", num, title,
                              s.get("solicitation_agency", "US Federal"),
                              "United States", "SBIR/STTR", "US-direct",
                              s.get("close_date", ""), link,
                              " ".join(t.get("topic_title", "")
                                       for t in (s.get("solicitation_topics") or [])
                                       if isinstance(t, dict))[:2000])
                op["number"] = num
                out.append(op)
        except Exception:
            break  # quiet best-effort
    return out


def fetch_uk_contracts():
    """UK Contracts Finder (OCDS), keyword-searched and pre-filtered to
    Aethl-relevant medical items so it doesn't flood the sheet."""
    out = []
    terms = [t.lower() for t in (STRONG_TERMS + MEDIUM_TERMS)]
    seen = set()
    for kw in ["wound care", "wound dressing", "tissue viability",
               "advanced wound", "diabetic foot", "burn injury"]:
        try:
            url = ("https://www.contractsfinder.service.gov.uk/Published/Notices/"
                   f"OCDS/Search?keyword={parse.quote(kw)}&size=50")
            d = _get_json(url)
            for rel in d.get("releases", []):
                ...
                if not any(term in blob for term in terms):
                    continue
                oid = rel.get("ocid") or rel.get("id")
                if oid in seen:
                    continue
                seen.add(oid)
                val = (t.get("value") or {})
                deadline = (t.get("tenderPeriod") or {}).get("endDate", "")
                op = make_opp("UK Contracts Finder", oid, title,
                              (rel.get("buyer") or {}).get("name", "UK public body"),
                              "United Kingdom", "tender/contract", "UK-tender",
                              deadline[:10] if deadline else "",
                              f"https://www.contractsfinder.service.gov.uk/notice/{rel.get('id','')}",
                              (t.get("description") or "")[:2000])
                op["number"] = str(oid)
                if val.get("amount"):
                    op["total_funding"] = to_int(val["amount"])
                out.append(op)
        except Exception:
            pass  # best-effort
    return out
...
def fetch_sam_gov(limit=50):
    # Collect one or more SAM keys: SAM_API_KEY, SAM_API_KEY_2.._5, or comma list
    raw = ([os.environ.get("SAM_API_KEY")]
           + [os.environ.get(f"SAM_API_KEY_{i}") for i in range(2, 6)]
           + os.environ.get("SAM_API_KEYS", "").split(","))
    keys = list(dict.fromkeys([k.strip() for k in raw if k and k.strip()]))
    if not keys:
        print("  [SAM.gov] SAM_API_KEY not set -> skipping", file=sys.stderr)
        return []
    ki = {"i": 0}

    def sam_get(base_url):
        """GET with api_key; on 429, rotate to the next key and retry."""
        while ki["i"] < len(keys):
            sep = "&" if "?" in base_url else "?"
            try:
                return _get_json(f"{base_url}{sep}api_key={keys[ki['i']]}")
            except error.HTTPError as e:
                if e.code == 429:
                    ki["i"] += 1   # this key is spent -> next key
                    continue
                raise
        raise error.HTTPError(base_url, 429, "all SAM keys exhausted", None, None)
    ...
    sam_fail = 0
    desc_cap = int(os.environ.get("SAM_DESC_CAP", "30"))
    # PASS 1: run all keyword searches first, so a small daily quota is spent
    # seeing opportunities from every term before any description fetches.
    pending_desc = []   # (op, description_url) to fill if quota remains
    for kw in ["wound care", "wound healing", "wound dressing", "burn injury",
               "diabetic foot", "tissue oxygenation", "chronic wound"]:
        if sam_fail >= 2 or ki["i"] >= len(keys):
            break
        try:
            base = ("https://api.sam.gov/opportunities/v2/search?"
                    + parse.urlencode({"limit": limit, "postedFrom": frm,
                                       "postedTo": to, "title": kw}))
            d = sam_get(base)
            for o in d.get("opportunitiesData", []):
                ...
                if award.get("amount"):
                    try:
                        ...
                        pass
                desc_url = o.get("description")
                if desc_url and desc_url.startswith("http"):
                    pending_desc.append((op, desc_url))
                out.append(op)
            sam_fail = 0
            time.sleep(0.5)
        except Exception:
            sam_fail += 1
    # PASS 2: spend any remaining quota pulling full descriptions for the AI.
    desc_fetched = 0
    for op, desc_url in pending_desc:
        if desc_fetched >= desc_cap or ki["i"] >= len(keys) or sam_fail >= 2:
            break
        try:
            dd = sam_get(desc_url)
            txt = dd.get("description") if isinstance(dd, dict) else str(dd)
            if txt:
                op["summary"] = re.sub(r"<[^>]+>", " ", txt)[:4000]
                desc_fetched += 1
        except Exception:
            sam_fail += 1
    if sam_fail >= 2 or ki["i"] >= len(keys):
        print("  [SAM.gov] rate-limited (daily key quota) -> skipped this run",
              file=sys.stderr)
    return out
...
CHANNEL_DEFAULTS = {
    ...
    "UK-tender":      (2,   4,   3,       3),   # UK procurement, needs local presence
    "foundation":     (2,   3,   2,       3),   # often needs academic partner; low indirect cap
}
...
def main():
    ...
    enrich_nih_pages(opps)
    if pypdf is None:
        print("  [PDF] pypdf not installed -> announcement PDFs not read "
              "(add 'pypdf' to the workflow pip install)", file=sys.stderr)
    else:
        print(f"  [PDF] read {PDF_READS['n']} announcement PDF(s) for funding detail",
              file=sys.stderr)
